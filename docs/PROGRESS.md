...
- 最后更新：2026-09-07
- 更新人：Codex
- 当前阶段：F001–F003 已验证；F004 处于透明 `provisional` 工程状态，F005–F017 已完成 provisional 工程单元并处于 `candidate_complete`；真实复核和正式 Goal Gate 仍待完成
- 最新本地工程检查点：`d00c4cec18db75d5642d941557ae547965fcf101`（F017 证据查看界面实现）
- F004 candidate 实现已推送并通过独立 CI；F005–F017 为本地 provisional 工程实现，真实年报语义/风险/报告准确率和正式 Goal Gate 仍待完成
- 测试状态：124/124 pytest 通过，覆盖率 90.89%；3/3 合成黄金用例通过；F006 专项 6/6、F007 专项 2/2、F008 专项 26/26、F009 专项 6/6、F010 专项 5/5、F011 专项 4/4、F012 专项 4/4、F013 专项 6/6、F014 专项 5/5、F015 专项 4/4、F016 前端契约测试 2/2、F017 定向测试 5/5 通过；Node JavaScript 语法检查通过；迁移契约测试通过，F017 未新增迁移
- 质量状态：F004 本地 Ruff、格式检查、严格 mypy、SQLite 升级、Alembic 零漂移和 PostgreSQL 17 独立 CI 均通过；F015 运行进度与 SSE 安全边界、F016 最小前端、F017 Claim/Evidence/来源页只读投影、PDF 完整性读取、用户隔离和不可定位原因披露均已通过本地门禁；真实中文年报准确率和证据覆盖率尚未验证
- 本次变更摘要：F003 已实现页级文本与哈希、不可变坐标索引、表格候选、结构化失败、幂等重放、用户隔离与页数上限；F004 已实现确定性三表识别、报告期/合并口径判定、结构化缺失与人工确认边界；F006 已实现 15 项 Decimal 指标、结果快照和计算 API；F007 已实现 Claim/Evidence 关系和来源定位校验；F008 已实现类型化工作流状态、确定性节点路由、状态转移审计和状态边界校验；F009 已实现确定性计算/限制/推断 Claim、metric/rule Evidence 自动挂接和幂等分析 API；F010 已实现 RiskFinding、版本化风险规则、事实与规则/指标 Evidence、数据质量降级和幂等风险 API；F011 已实现 Report、financial-report-v1 Schema、证据映射、只读组装和幂等报告 API；F012 已实现独立 Evaluation、结构化检查/修复指令、输入快照和幂等评测 API；F013 已实现唯一 Goal Gate、GoalGateDecision、verified 状态保护和失败修复路由 API；F014 已实现类型化初始快照、版本化 Checkpoint 保存/恢复、来源哈希完整性校验、用户隔离和恢复审计幂等；F015 已实现用户范围运行进度查询、任务与错误摘要、有限 SSE 生命周期事件、Last-Event-ID 重连和安全 payload 白名单；F016 已实现 FastAPI 内置静态前端、任务创建、PDF 上传、真实运行进度和有限生命周期事件展示；F017 已实现报告 Claim/Evidence 与有界片段、来源文件及原 PDF 页面的安全跳转。
...
- F004 双人复核与正式 Goal Gate 外部验收尚未完成；F005–F017 仍是 provisional 工程证据，真实年报事实/风险/报告准确率、真实证据覆盖率和端到端完成率不得声明；F018 合成黄金流程端到端验收待执行。
...
- F016 当前是无构建静态 UI，只复用已有创建运行、PDF 上传、进度和有限 SSE 接口；尚未实现生产认证、完整工作流自动执行或 F017 页级证据跳转。
...
完成 F017 文档与 Git 检查点后，按项目负责人授权实现 F018 合成黄金流程端到端验收；保留 F004–F017 的正式 Goal Gate 外部证据阻塞、人工复核要求和真实年报准确率禁令。
