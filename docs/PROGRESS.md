...
- 最新本地工程检查点：`62d2f3d5df91de6b9c53494687b8e92f7c92b62e`（F018 合成端到端验收与 F005/F006 概念契约补齐）
...
- 完整前端工作台已接入用户隔离运行列表、真实分阶段操作、人工事实确认、指标谱系、风险、结构化报告、证据 PDF 跳转、Evaluator、Goal Gate 和 Checkpoint 恢复；正在进行最终专项门禁和交接检查。
...
执行 F016、F017、F018 专项门禁和最终完整验证，确认 Git 工作区干净。工程交付后仍需由真实独立 Reviewer A/B 执行 F004 盲审与冲突裁决；在证据到位前不得把 provisional/candidate 状态改写为 `verified`。
