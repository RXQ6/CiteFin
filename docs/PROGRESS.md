...
- 最后更新：2026-09-07
- 更新人：Codex
- 当前阶段：F001–F003 已验证；F004 处于透明 `provisional` 工程状态，F005–F008 已完成 provisional 工程单元并处于 `candidate_complete`；真实复核和正式 Goal Gate 仍待完成
- 最新本地工程检查点：F008 正在提交；此前检查点为 `c61b9f7`（docs: record F007 checkpoint — preserve evidence workflow state）
- 最新 F004 candidate 实现已推送并通过独立 CI；F005–F008 为本地 provisional 工程实现，真实年报语义准确率和 Goal Gate 仍待完成
- 测试状态：87/87 pytest 通过，覆盖率 91.28%；3/3 合成黄金用例通过；F006 专项 6/6、F007 专项 2/2、F008 专项 26/26 通过；迁移升级和 Alembic 零漂移通过
...
- 本次变更摘要：F003 已实现页级文本与哈希、不可变坐标索引、表格候选、结构化失败、幂等重放、用户隔离与页数上限；F004 已实现确定性三表识别、报告期/合并口径判定、结构化缺失与人工确认边界；F006 已实现 15 项 Decimal 指标、结果快照和计算 API；F007 已实现 Claim/Evidence 关系和来源定位校验；F008 已实现类型化工作流状态、确定性节点路由、状态转移审计和状态边界校验。
...
- F004 双人复核与 Goal Gate 尚未完成；F005–F008 仍是 provisional 工程证据，真实准确率和真实证据覆盖率不得声明；按当前指令 F008 完成后暂停，不启动 F009 及其他后续功能。
...
完成 F008 的文档与 Git 检查点后暂停；后续如恢复工作，再由项目负责人明确选择 F009 及其后的功能，并保留 F004–F008 的正式 Goal Gate 阻塞和真实年报准确率禁令。
