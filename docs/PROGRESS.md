...
先由项目负责人解决 `manifest.json` 中仍为 pending 的公开披露使用依据，并把空白包分别交给真实独立 Reviewer A/B；两份完整提交返回后只对冲突行安排第三位裁决人，再运行 `validate-f004-review`。在这些外部证据到位前不得把 provisional/candidate 状态改写为 `verified`。前端后续可补生产认证、完整自动执行器和浏览器矩阵验证。
