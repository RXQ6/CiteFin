...
## 2026-09-09：公开站 UI 质感升级复核

### 执行方式与结果

- `scripts/dev.ps1 check`：后端 168 项测试通过，覆盖率 90.03%；前端 TypeScript 检查、Vitest 与 Vite 生产构建通过。
- 浏览器人工复核：`http://127.0.0.1:8000/` 首页可打开，标题、示例报告入口、关键指标预览、能力卡片与三步说明正常显示。
- 视觉检查覆盖：首页层次背景/玻璃导航/悬浮预览卡、按钮反馈、报告卡片和弹窗动效；同时保留 `prefers-reduced-motion` 降级路径。

### 结论与限制

- 本次为前端视觉与交互层升级，不改变财务计算、证据链、Goal Gate 或认证边界。
- 尚未执行 Playwright 多视口像素回归；移动端通过响应式规则与构建检查，正式发布前仍需在 1440px、768px、390px 做截图对比。

## 2026-09-09：本地产品站启动检查
...
## 2026-09-11：F019 报告数据可视化

### 基线与实现检查

- Windows 等价首次运行 `scripts/dev.ps1 setup` 成功：Python 74 个包已检查，前端 223 个 npm 包已安装。
- 改动前全量 pytest 为 168/168 通过，覆盖率 90.03%。
- 针对性后端用例 17/17 通过，覆盖图表生成、缺失/冲突省略、报告/API/工作台、迁移、Evaluator 和 Goal Gate；子集命令因全仓覆盖率门禁不适用于局部执行而返回非零，实际用例无失败。
- 前端 TypeScript 与 Vitest 检查通过（1/1）。

### 数据与安全边界

- 图表数值保存为十进制兼容字符串，并从 `FinancialFact`、`CalculatedMetric` 或 `RiskFinding` 显式引用来源。
- `data_snapshot_hash` 对规范化数据集计算 SHA-256；Evaluator 可识别被篡改快照、非法规范、缺失证据或报告引用不一致。
- 缺失指标、重复当前期事实、币种不一致或无风险发现时不生成对应图表，不以零值、插值或模型猜测补齐。
- 风险分布图明确披露其为规则命中数量，不代表发生概率、投资评级或买卖建议。

### 最终门禁

- `scripts/dev.ps1 check` 通过：Ruff、101 个文件格式、严格 mypy（59 个源码文件）、3/3 合成黄金用例、171/171 pytest、90.03% 覆盖率、1/1 Vitest 和 Vite 生产构建均成功。
- `scripts/verify_f019.py` 通过：6/6 专项服务/API 用例成功。
